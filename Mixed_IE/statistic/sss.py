...
def descend_list_sim( task_list, ret_list, index, fields, save_dict ):
    num = index
    index = index%8
...
def norm(save_dict):
    ret_dict = {}
    max_min = [[0, float('inf')] for _ in range(7)]
    for i in range(len(max_min)):
        ...
        spans.append( max_min[i][0]-max_min[i][1] )
        mins = max_min[i][1]
        for key, value in tqdm(save_dict.items(), desc="Norm"):
            if i==0:
                ret_dict[key] = []
            ret_dict[key].append( (value - mins)/(spans+1e-10) )
    return ret_dict
...
def main():
    ...
    with mp.Manager() as manager:
        ...
        task_end = mp.Queue()
        Process_Pool = [ mp.Process(target=descend_list_sim, args=(task_list_start[i], task_end, i , fields, save_dict )) for i in range(work_num)]
        ...
        quit_num = 0
        alive = [ 1 for _ in range(work_num)]
